 ...
 * `when`-derived pure-state idiom of [works.mees.jiib.ui.route.derive] — no CameraX, no Context, no
 * `rememberLauncherForActivityResult` (that Activity-Result/Compose glue is net-new and lands in
 * 11-06's CameraPermission wiring). Host-testable ([works.mees.jiib.spool.ScanStateMachineTest]).
 *
 ...
 * [works.mees.jiib.ui.route.derive]). Arm order is load-bearing — permission/hardware gates win
