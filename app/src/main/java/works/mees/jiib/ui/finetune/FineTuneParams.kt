...
import works.mees.jiib.command.FanArgs
import works.mees.jiib.command.FlowFactorArgs
import works.mees.jiib.command.PressureAdvanceArgs
import works.mees.jiib.command.PrinterCommands
import works.mees.jiib.command.RetractionArgs
 ...
 * [works.mees.jiib.theme.ThemeTokens.pool] — no text labels; the pool color on the leading icon
 ...
 * [works.mees.jiib.command.TrailingCommitBatcher]; markPending fires once per commit.
 *
 ...
 * filter key. Derived from the SAME [CommandRegistry] specs via [works.mees.jiib.command.CommandSpec.dispatchKey]
 * (dummy args — every key lambda depends only on the args' field constant, never the value), so a
 * registry key rename can never silently desync the [works.mees.jiib.command.CommandDispatcher.rejectedKey]
