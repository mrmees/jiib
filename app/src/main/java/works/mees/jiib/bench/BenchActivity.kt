 ...
 *   adb shell am start -n works.mees.jiib/.bench.BenchActivity --es scene compose
 *   adb shell am start -n works.mees.jiib/.bench.BenchActivity --es scene views
 *   adb shell am start -n works.mees.jiib/.bench.BenchActivity --es scene render
 *
 * The `render` scene (added in plan 03-07) is the D-10 perf scene: it mounts ONLY the two shared
 * render primitives (Compose [works.mees.jiib.render.ProgressRing] + classic-Views
 * [works.mees.jiib.render.GraphView] via [works.mees.jiib.render.GraphViewHost]) driven by the
