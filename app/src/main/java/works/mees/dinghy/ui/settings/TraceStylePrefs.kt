 ...
 * ([works.mees.dinghy.config.Profile.id]) — the [WebcamPrefs] precedent ("re-key on the profile id,
 ...
 * calls them from a composition scope — it routes through [works.mees.dinghy.di.AppContainer], which
...
class TraceStylePrefs(
     ...
     * entry means the sensor uses its default [works.mees.dinghy.theme.SeriesColor.seriesColor] token.
