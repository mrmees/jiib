...
---
*Last updated: 2026-06-12 — Phase 28 (System / Settings Cluster, jiib REDESIGN) COMPLETE & verified (5/5 SC; owner UAT 6/6 on flox + 5/5 post-review-fix re-walk approved). The System cluster restyled onto the jiib kit and the P24 nav story finished: real System page (NavDest.System, static brand Focus capped 20%H/40%W, direct-tap rows Printers→Settings→Theme→SysInfo→About→Power-stub), swipe-up gesture + AppDrawer DELETED entirely (D-04; ShellPresence/DrawerWebcamGating/FineTuneNav androidTests retired with it), Temperature/Console/Fine-Tune rehomed to the home idle list (8→11 rows), System tile added to the printing shortcut grid, maxItems persistence axis deleted (load-bearing Palette.generate/TemperatureScreen paths preserved), theme editor gained hue+S/V-square full-color picking with ARGB→HSV restore + reactive idle seed (the REAL bug was ThemeEditorScreen.kt:117, not AppContainer) + dev-cycler drag restored, Printers rebuilt with R4 Edit/Delete arming mode toggles + ConfirmGuard. ⚠ UAT GAP-A: the UI-SPEC flattened D-09 "sub-1U rows" into fixed 10dp padding and the whole dense-row concept shipped off-grid — owner ruling "All 1U": dense param DELETED, every row/control back on heightIn(min=uDp), LAYOUT.md C6 clause rewritten (density = grouping, never row height; Settings now scrolls slightly past one page at M, owner-accepted). Codex pre-exec review caught 2 show-stoppers AGAIN (PrintStatusScreen onOpenDrawer 6-callsite compile-break; 3 orphaned drawer androidTests). Post-UAT code review 1 Critical / 9 Warning / 10 Info → ALL 10 C+W fixed same-day (52e7c17..838b79d; CR-01 = Save resurrecting a cleared API key from a stale snapshot). D-21 owner glyph picks: Fine-Tune=line_style, Printers=android_wifi_3_bar_plus (rest carried from drawer). Closed todos: retire-swipe-up-nav, settings-densify, printers-edit-delete, 15.1-review-deferred, dev-overlay-drag (phase-14 WR-03 webcam null-key stays pending → P29). Next: Phase 29 (Release Hardening & Ship — LAST). Prior: 2026-06-11 — Phase 26 (Adjustment Screens, jiib REDESIGN) COMPLETE & verified (5/5 must-haves; owner UAT on flox: 4 pass / WR-08 deferred to todo). Seven plans / 5 waves: 26-01 built the shared `IncrementPicker` + `AdjusterPanel` design-system components (closing the C2/D-20 todo); 26-02 collapsed the four Fine-Tune screens into ONE flat-list screen on the adjuster archetype (−1290 LOC; P17 clamp/busy-lock discipline preserved, test-proven); 26-03/26-04 rebuilt Temperature — per-sensor trace color + show/hide persisted via a new `TraceStylePrefs` DataStore (writeScope discipline), a Focus that MORPHS graph↔adjuster, Colorful-8 swatch row with equality-guarded `GraphView.setTraceColorOverrides`, Field-takeover presets, Cooldown + per-heater Off through `clampHeaterTarget`; 26-05 collapsed the three Outputs detail pages into detail-in-Focus (`OutputFocusControl`; build-once scrubber rule kept); 26-06 brought Extrude to conformance (FootButtonBar, numeric-IME distance/speed, filament-preset Field-takeover applying extruder temp only, D-15 layout-shape exemption kept); 26-07 retired `NumpadPage` + `MeasuredWeightPage` app-wide (macro params + measured weight on numeric IME — D-07/D-08 done). Code review: 4 Critical / 11 Warning / 7 Info — ALL 4 Critical + 10/11 Warning FIXED same-day (commits e088610..bc4a864; CR-01 frozen-snapshot adjuster, CR-02 off-heater unheatable, CR-03 fmtValue 1500→"15" corruption, CR-04 stale-scrubber-closure cross-output dispatch — the fa97efb class resurfacing, killed by key(objectKey) + in-place reseed). Open follow-ups (tracked todos): WR-08 owner must assign 4 distinct fw-retraction glyphs; Temperature adjuster fire-without-echo-wait option (owner UAT note — rapid ± presses currently drop while awaiting echo, acceptable for now); owner's "finer detail points + new formatting rules" from UAT pending enumeration. Next: Phase 27 (Motion + Calibration) — `/gsd-discuss-phase 27`. Prior: 2026-06-10 — Phase 25 (Browse Screens, jiib REDESIGN) COMPLETE & verified (5/5 must-haves; owner-approved on-device UAT on flox; code review 2 critical / 8 warning / 5 info — ALL 10 Critical+Warning FIXED same-day via the code-review fix pass, commits cd96fe2..cde6725). Four browse screens migrated onto the Phase-23 component kit: Files (Compose ListBlock/ListRow per the binding 25-01 spike PASS, p90 16.42ms vs 41.95 Views baseline), Console (Field-only; KEEPS the RecyclerView scrollback — Compose FAILED the spike at ~8x p90 under live churn — with FootButtonBar filter toggles + render-time filtering), Macros (three surfaces — Bookmarked + System + ExecutionPopup — collapsed into one MacroFieldMode screen with typed param sanitizer), Webcam (D-17 crash confirmed already dead since Phase-21 49f3fe2 via live repro; the REAL fix was WR-02: AppShell read the write-dead connectionStore.config — now container.activeConfig). 9 owner-sanctioned DinghyIcon tokens registered (Execute=play_arrow + Unbookmarked=radio_button_unchecked owner-picked at the D-21 gate); verify_ligatures 87/87. Post-review owner adjudications: Files-local e-stop REMOVED (was a no-op confirm, AppShell overlay covers it); Webcam full-focus RESTORED (50/50 regression) and re-verified live on flox. Closed todos: webcam-screen-crash, webcam-tile-gating, files-delete-gating-too-broad (old Phase-7 defect — 25-03 DeleteGate scopes Delete to the printing file, UAT D-08 confirmed). Spike harness torn down. Next: Phase 26 (Adjustment Screens). Prior: 2026-06-08 — Phase 20 (System Information Page) COMPLETE & verified (4/4 must-haves; code review 0 blocker / 2 warning — BOTH fixed; SYS-01..05 complete). A read-only host-health/diagnostics page for the printer's SBC, host-health-only per the D-13 rescope (versions live in About; disk/network out of scope). Built TDD across 4 sequential waves: (01) five live-captured cross-SBC fixtures encoding the three real traps (push omits throttled_state+system_uptime; RockPro64 sends throttled_state:null and empty cpu_info.model "") + five compiling RED scaffolds + the icon-ligature drift gate extended with the owner-locked Phase-20 glyphs; (02) pure host-tested logic — tolerant models/parsers, the throttle-authoritative-else-temp-fallback healthState() decision fn (D-12: warn ≥70°C / caution ≥80°C), kB/uptime formatters degrading to "—"; (03) spine plumbing — routed the previously-dropped free 1 Hz notify_proc_stat_update push, registered machine.system_info + machine.proc_stats query specs cleanly through the command-catalog drift gate (catalog.json + printer-matrix.json — the [[dinghy-command-catalog-drift]] trap NOT tripped), two edge-driven one-shot handshake seeds (NO poll loop, honoring the Phase-13 cadence contract), and a dedicated SystemInfoHolder off the printer hot path; (04) the read-only Compose screen (icon-led InfoRows + shape-coded health chip) + the live drawer tile flipped atomically with its AppShell route. On-device cross-SBC UAT owner-APPROVED on flox against BOTH real printers (RPi 4/Ender 5 Plus + RockPro64/Ender 3) — page, 1 Hz live cadence, and a semantically-identical health chip verified; the sole owner observation (Back → home, not the drawer) was confirmed as the app-wide by-design caller-return nav grammar (ShellNavState.navigateTo/goBack — every drawer screen returns to its caller, the drawer is a transient overlay) and accepted as-is, NOT a defect. Code-review warnings both fixed at HEAD: WR-01 (SystemInfoHolder leaked one idle push collector per reconnect → added a cancel() handle torn down on rebuild) + WR-02 (a non-primitive flags element threw and collapsed the entire proc_stats query → safe per-element cast). Next: `/gsd-discuss-phase 21` (WebRTC Camera Streaming). Prior: 2026-06-07 — Phase 18.2 (jiib Rebrand) COMPLETE & verified (5/5 must-haves; code review 0 critical / 3 warning / 3 info). User-facing brand swapped "Dinghy Display" → **jiib** (repo + package + applicationId unchanged): 3 source SVGs → tintable VectorDrawables (launcher foreground fixed-white; wordmark + lockup single-fill), launcher adaptive icon rewired (v26 + monochrome + pre-26 fallback, navy bg retained, benchy mark gone), app label + FGS notification title → @string/app_name, Splash lockup + About wordmark/tagline/jib-note accent-tinted via a new brandTint WCAG-3:1 contrast-floor helper (host-tested), debug GalleryScreen header de-branded. SC-4 closed via an exhaustive DEBRAND-AUDIT (only out-of-scope survivors: MoonrakerSession clientName — JSON-RPC server-side identity, conscious retention — + source comments); assembleRelease + full host unit suite GREEN (SC-5); on-device flox spot-check (launcher/label/recents/notification/splash/about) owner-APPROVED on real hardware. Open follow-ups from code review (out of this phase's name-swap scope): WR-02 FGS notification still wears a Bluetooth small-icon glyph (needs a 24dp jiib status icon); WR-03 brandTint text-fallback invariant for custom themes. Next: `/gsd-discuss-phase 18.3` (color-reactive spool glyph — needs a design pass first). Prior: 2026-06-06 — Phase 15.2 (Theme Conformance Sweep & Settings IA) COMPLETE & verified (28/28 must-haves; code review 0 critical / 4 warning / 3 info — both recurring traps [write-scope cancellation, override purity] verified handled). Delivered: transient theme-override layer + dev style/size/printer cyclers (the conformance accelerant), four-tile Settings IA dissolve (Printers · Theme · Settings · About) with the Appearance controls relocated onto the Theme tile, app-wide Back→Neutral sweep, a D-10 guided core-screen review that hardened the LAW (criteria C1–C7 into THEMING.md/LAYOUT.md), and the finalized 32-surface AUDIT.md scorecard with mechanical fixes applied (Move accent jog + force-move stop-fill, Temp context presets/cooldown, Files print=accent, long-tail fsSp floors, bed-mesh heatmap ramp made theme-derived low=pool/high=accent). All six plans' on-device checkpoints owner-verified live on flox + real printers. Resolved the settings-vs-devices + pool-color-semantic todos. Owner-deferred (tracked todos): increment-picker 3-cell component + Move-Z layout (P17), settings densify-restyle (P20), Printers edit/delete-mode buttons (P16), webcam crash + webcam-URL connectionStore bug (P20), bookmarked-macros density + 15.1 WR-01/WR-02 theme-editor findings (P21). NOTE: phase.complete auto-advanced STATE to 17 (Phase 16 has no plans yet); corrected to Phase 16 (Home/Print-Status Redesign) per ROADMAP order. Next: `/gsd-discuss-phase 16`. Prior: 2026-06-04 — Phase 13 (Optimization, Network Efficiency & End-to-End Reliability) COMPLETE: the promoted reliability phase. Capture-first (live SAVE_CONFIG wire-captures on both printers) → hardened the mock-that-lied → visible self-healing klippy-restart recovery (the headline freeze, dead) → request-cadence contract + refreshProbeZOffset removal → binding dual-printer on-device UAT. UAT round 1 caught two live bugs green units missed (G-A1 Home-bounce; G-B1 silent mid-print WiFi-drop = no OkHttp pingInterval — the 4th mock-vs-reality strike); 13-05 gap-closure fixed both (keepalive + nav-hoist + full Syncing splash on socket reconnect, a deliberate D-05 departure) and UAT round 2 PASSED on flox + live E5 AND E3. Verification 4/4; code review 0 critical (3 warnings fixed). Codex caught a real per-drop-watchdog bug in the headline fix. Next: Phase 14 (Release Hardening & Ship) OR resume Phases 6/7 (In Progress) / 10–12 (deferred features). Prior: 2026-06-03 — Phase 9 (Calibration & Maintenance) complete: bed-mesh/Z-tilt/QGL/screws-tilt + the interactive Probe-Calibrate manual-probe Z session, all capability-gated behind one Calibration drawer tile; D-15 delete-scoping. On-device UAT PASSED on flox + live Ender 3. Probe-Calibrate substantially reworked on device (manual_probe partial-diff fix, macro-feedback hero, three-value display, home gate, dismissable toasts). **Roadmap promotion (2026-06-03):** Phase 13 (Optimization/Network-Efficiency/End-to-End-Reliability) PROMOTED to run NEXT (execution-order only; number unchanged) ahead of Webcam/Spool/Macro-Prompt — driven by a Phase-9 UAT finding that the SAVE_CONFIG re-handshake freezes the live feed (a session-layer reliability bug, deferred to Phase 13 as its headline task). Next: `/gsd-discuss-phase 13`. NOTE: this Active list still lags reality — Phases 4–9 shipped but their Active items aren't all moved to Validated; a fuller reconciliation (or `/gsd-docs-update`) remains due.*
