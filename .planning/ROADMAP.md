...
- [x] **Phase 17: Fine-Tune / Live-Adjust Panel** - A lean live-adjust tuner organized by failure-mode — **Motion** (speed M220, accel/max-velocity/SCV) and **Extrusion** (flow M221, pressure advance, firmware retraction if present, part-cooling fan) — wired from the stubbed Print-Status Tune button; capability-gated, keyboard-free, always-available; temps/pause linked not duplicated. (Z babystep moved to Phase 16; object exclusion → v2.) See 17-CONTEXT.md. (completed 2026-06-08)
...
- [ ] **Phase 23: Interaction Coherence & Page Overhauls** (INSERTED 2026-06-08) - Design/creative pass on the disjointed look/feel: consistent interaction grammar across screens + major overhauls of the pages that need them. Kept separate from the mechanical conformance sweep (Phase 24) for clean verification. (22↔23 order may flip per the audit if the slow screens ARE the overhaul screens.)
- [ ] **Phase 24: Touch-Target / Scaling / Rotation Conformance Sweep** (INSERTED 2026-06-08) - Full-app mechanical pass on touch targets (≥64px), S/M/L text scaling, and portrait/landscape rotation — folding in the old Phase-18-deferred tokenization/icon-call-site backfill + the light late-surface conformance sweep.
- [ ] **Phase 25: Release Hardening & Ship — Always-On, Lifecycle & Signed APK** (RENUMBERED from 22, 2026-06-08) - The deferred print-loop robustness (reconnect print-state resync + process-death recovery) PLUS full Doze/always-on survival, burn-in screensaver, the "looks done but isn't" checklist, R8 release build, PKG-01/03, and a signed sideloadable APK shipped via GitHub Releases on a real Nexus 7 — ships v1 (LAST phase).
...
- [x] 15.2-04-PLAN.md — Wave 4: 4-tile IA split (Printers · Theme · Settings · About) + Dest/DRAWER_TILES/when(dest) routing + About dev-enable toggle (D-01/D-03/D-04/D-05)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 15.2-05-PLAN.md — Wave 5: app-wide Back-intent sweep to Neutral + D-10 guided core-screen review → harden THEMING.md + AUDIT checklist (D-10/D-13)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 15.2-06-PLAN.md — Wave 6: app-wide AUDIT.md scorecard across all ~30 surfaces + long-tail mechanical conformance fixes + judgment-call gate (D-11/D-12/D-13)
...
- [x] 16-01-PLAN.md — RED scaffolds (compile-clean): PrintStatusModeTest, PreheatTest, GlanceSensorTest, BabystepPrefsTest, + extend PrinterCommandsTest / PrinterStateReducerTest
...
- [x] 16-06-PLAN.md — PrintStatusScreen four-mode rework off the classifier + pure PrintStatusUiModel seam + AppShell launcher nav + spool-aware Preheat (capability-gated) + babystep row + Dismiss/Reprint
...
- [x] 17-01-PLAN.md — RED test scaffolds (compile-clean): extend PrinterCommandsTest/CommandRegistryGcodeTest/PrinterStateReducerTest + new FineTuneHolderTest + instrumented FineTuneNavTest (TUNE-01..07)
...
- [x] 17-08-PLAN.md — GAP 2 (Check 8, MINOR): ShellNavState.navigateTo runs per-dest entry resets on same-dest re-selection so Fine-Tune (and Calibration/Macros) reset to Hub on re-entry; turns the RED instrumented FineTuneNavTest GREEN (autonomous:false; on-device flox) (TUNE-01)
...
- [x] 18-03-PLAN.md — Icon-token registry (DinghyIcon/IconRef/View) + strings.xml master + convention
...
- [x] 18-06-PLAN.md — Exemplar #2 FineTune (capability-gating + busy-lock archetype)
...
- [x] 18.1-04-PLAN.md — Delete 13 superseded drawables + KDoc doc-link fixes + full no-regression gate + on-device spot check
...
- [x] 18.2-02-PLAN.md — App label + FGS notification title -> "jiib" via @string/app_name; seed About tagline + jib-note strings (D-09/D-10/D-11)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 18.2-03-PLAN.md — Wire the jiib launcher icon (adaptive v26 + monochrome + pre-26 fallback), retain navy bg (D-01/D-02/D-03)
- [x] 18.2-04-PLAN.md — Splash lockup + About wordmark+note, accent-tinted via brandTint contrast-floor helper; brand preview matrix (D-04/D-05/D-06)
...
- [x] 19-05-PLAN.md — OutputsHolder + flat Outputs list + preview matrix

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 19-06-PLAN.md — Per-type detail pages (scrubber/LED+Off D-12/pin toggle), immediate settle-dispatch
...
- [x] 19-10-PLAN.md — GAP-B: per-LED channel-capability detection + white-only brightness-slider page (WHITE channel) vs RGB wheel (SC-2/SC-3)
...
- [x] 20-02-PLAN.md — Pure logic: system_info/proc-stat parsers + health-chip decision fn (throttle vs D-12 temp-fallback) + kB/uptime formatters (SYS-01..04)
