...
- [ ] 02-02-PLAN.md — Pure state layer: PrinterStateReducer diff-merge (STATE-01), deriveCapabilities (STATE-02), Klippy lifecycle (STATE-04) — unit-tested over golden frames
...
**Success Criteria** (what must be TRUE):
  ...
  2. App shows a splash during Klippy startup, routes to the main menu on Klippy ready, and routes to job status when a print is active — the route is driven by `klippy_state`, not socket state; the splash surfaces shutdown/error reason text with recovery actions (retry, firmware/Klipper restart where exposed)
  3. The main menu doubles as a launcher and a compact thermal dashboard (heater rows + a live temperature value/mini-graph) that proves the SHARED render/throttle primitive — rendering smoothly at the throttled ~2–4 Hz cadence on the Nexus 7; this primitive is what the Temperature panel later EXTENDS into the full history graph, so graph architecture is settled here, not discovered late
  4. Emergency Stop is reachable from the shell on every screen, triggers `printer.emergency_stop`, and is fast-but-deliberate (hold/double-tap) rather than a slow modal; connection/printer status is always visible in the shell chrome
